      ...
      <div className="container">
        ...
        <Routes>
          ...
          <Route element={<ProtectedRoutes isSignedIn={user} />}>
            {user == "Customer" ? (
              customerRoutes.map((item) => <Route key={item.id} path={item.path} element={item.element}></Route>)
            ) : user == "resturantManager" ? (
              restaurantRoutes.map((item) => <Route key={item.key} path={item.path} element={item.element}></Route>)
            ) : (
              <Route path="" element={<LandingPage />}></Route>
            )}
          </Route>
