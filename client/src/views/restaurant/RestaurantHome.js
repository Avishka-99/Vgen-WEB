...
export default function RestaurantHome() {
	...
	const getOrderTypeDetails = async () => {
		try {
			const res = await Axios.get(API_ENDPOINTS.GET_ORDER_TYPE_URL, {
				params: {
					user_id: user_id,
				},
			});
	...
	const getOrderCountDetails = async () => {
		try {
			const res = await Axios.get(API_ENDPOINTS.GET_ORDER_COUNT_URL, {
				params: {
					user_id: user_id,
				},
			});
				...
				<div className='Details-left'>
					...
					<div className='table-details'>
						<div className='table-detail-header'>
							...
							<button id='filter-order' onClick={() => setFilterOrder(true)}>
								Filter order
							</button>
						</div>
									...
									<tbody>
										{orders.map((o) => (
											<tr key={o.orderId}>
												...
												<td>{o.orderId}</td>
												<td>{o.orderType}</td>
												<td>{o.status === 1 ? <p style={{color: 'green'}}>complete</p> : <p style={{color: 'red'}}>pending</p>}</td>
												<td> {o.orderState === 2 ? <p style={{color: 'green'}}>complete</p> : o.orderState === 1 ? <p style={{color: 'orange'}}>preparing</p> : <p style={{color: 'red'}}>pending</p>}</td>
											</tr>
										))}
									</tbody>
								</table>
					...
					{filterOrder ? (
						<>
							<div className='filter-order'>
								<button id='filter-order' onClick={() => setFilterOrder(false)}>
									Back
								</button>
								<br />
								<p>Order Type </p>
                  ...
                  <br /> */}
						...
						<>
							<div className='most-ordered'>
								...
								<div className='most-ordered-content'></div>
							</div>
							<div className='barChart'>
								...
								<div className='barChart-content'>
									{isLoading ? (
										...
										<>
											<PieChart data={pieChartData} lineWidth={50} />
