...
const ManufactureHome = () => {
  ...
  const searchForChart = async (value) => {
    try {
      const res = await Axios.get(API_ENDPOINTS.getRawSearchForChart_URL, {
        params: {
          user_id: user_id,
          value: value
        },
  ...
  const searchForValue = async (value) => {
    try {
      const res = await Axios.get(API_ENDPOINTS.getRawSearchForValue_URL, {
        params: {
          user_id: user_id,
          value: value
        },
  ...
  const orderDetailsSearch = async () => {
    try {
      const res = await Axios.get(API_ENDPOINTS.getRawSearchData_URL, {
        params: {
          user_id: user_id,
        ...
        {filter ? (
          <>
            <div className="manufacture-filter-order">
              ...
              <DatePicker selected={startDate} onChange={(date) => setStartDate(date)} selectsStart startDate={startDate} placeholderText='Order Date' />
              {/* <br />
                      ...
                      <br /> */}
              <button id='manufacture-search-order' onClick={() => orderDetailsSearch()}>Search Order</button>
            </div>
          </>
        ) : (
          <>
